...
test = (args..., expect, message) ->
  try
    ...
    throw e
  console.log sprint '%04.3d: OK %s', ++i, message

test 'Message', 'Message',
     'sprintf() without arguments'
test '%$s %6. 6s %6.6 s', '%$s %6. 6s %6.6 s',
...
test '%d', 123456.789, '123456',
     "%d format shouldn't round"
test '%d', -0, '0',
     "%d shouldn't represent negative 0"
test '%-03.2d', 1, '01 ',
...
test '%*.*f', 6, 1, 12.34, '  12.3',
     'Star (*) operator'
test '%3$0*1$.*2$f', 8, 2, 3.123, '00003.12',
     'Star (*) operator with argument number'
test '%3s', 'test', 'test',
...
test '%.0f', 0, '0',
     'Floats and 0'
test '%.0f', Math.pow(2, 38), '274877906944',
     'Very big floats'
test '%.1f', 3.14, '3.1',
     'Accuracy'
test '%.1f', 3.16, '3.2',
     'Rounding'
test '%4.1f %.0f %.0f', 4.123, 0.6, -0.6, ' 4.1 1 -1',
...
test '%06.5b', 12, ' 01100',
     '0 flag with precision should round to precision.'
test '%.5b', 12, '01100',
     'Binary numbers with precision bigger than length'
test '%.0b %+.0b % .0b %-.0b', 0, 0, 0, 0, ' +   ',
     '0 with 0 precision'
test '%#3.0b %#3.4b', 1, 1, '0b1 0b0001',
     'Precision and prefix at once.'
test '%c', 0x21, '!',
     '%c format'
test '%c', "\x00",
     '%c format without argument'
test '%1$3c %1$03c', 65, '  A 00A',
     '%c format with width specified'
test '%vd %vd', "AB\u0100", "\x01\x02\x03", '65.66.256 1.2.3',
     'Vector flag'
test '%*vX', ':', "\x42\x42", '42:42',
     'Custom separator character'
test '%2$*1$vd', ':', "\x7F\x00\x00\x01", '127:0:0:1',
     'Vector flag with custom separator on specified position'
test '%#v.8b', "\x61\x00\x62", '0b01100001.00000000.0b01100010',
     'Binary vectors'
test '%v.0o', "\x01\x00\x02\x00", '1..2.',
     'Octal vectors'
test '%#v.0o', "\x01\x00\x02\x00", '01.0.02.0',
     'Octal vectors with prefix'
test '%vs', 'Hello', '72.101.108.108.111',
     'Octal strings'
test '%vc', 'Hello', 'H.e.l.l.o',
     'Octal characters'
test '%*2147483647$v2d', '', '',
     'Invalid octal parameter'
